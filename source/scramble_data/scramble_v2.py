"""
DA USARE quando ho una cartella piena di cartelle piene di immagini

INPUT
/dump/cartella1/immagine.png
/dump/cartella2/immagine.png
...
path_images_input = "/spoofing-detection/dump/"  # Definisce il percorso della cartella di input contenente le immagini
category = ["spoof", "real"]  # Crea una lista di categorie per le immagini
category_target = category[1]
...
for i in list_path_images_input:  # Itera attraverso ogni elemento nella lista dei percorsi delle immagini
    ...
    path_single_folder_list = os.listdir(path_single_folder)  # Elenca i file nella cartella corrente

    for j in path_single_folder_list:  # Itera attraverso ogni file nella cartella corrente

        absolute_path_start = working_directory + path_images_input + i + "/" + j  # Costruisce il percorso assoluto del file
        ...
        final_path = f"spoofing-detection/input/lcc-fasd/LCC_FASD/LCC_FASD_{target_directory}/{category_target}/" + j  # Costruisce il percorso finale per il file
